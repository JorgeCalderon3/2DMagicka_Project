...
public class Bush extends StaticEntity {
    ...
    public Bush(Handler handler, float x, float y) {
        super(handler, x, y, Tile.TILEHEIGHT*2, Tile.TILEWIDTH*2);
        bounds.x=14;
        bounds.y=50;
        bounds.width = 36*3;
        bounds.height = 55;
    ...
    public void die() {
    	...
    	int nItems = rand.nextInt(3)+1;
    	switch(nItems) {
    	case 1:
    		handler.getWorld().getItemManager().addItem(Item.stickItem.createNew((int)x + bounds.x + 32,(int)y + bounds.y,1));
    		break;
    	case 2:
    		handler.getWorld().getItemManager().addItem(Item.stickItem.createNew((int)x + bounds.x + 32,(int)y + bounds.y,1));
    		handler.getWorld().getItemManager().addItem(Item.stickItem.createNew((int)x + bounds.x + 16,(int)y + bounds.y,1));
    		break;
    	default:
    		handler.getWorld().getItemManager().addItem(Item.stickItem.createNew((int)x + bounds.x + 32,(int)y + bounds.y,1));
    		handler.getWorld().getItemManager().addItem(Item.stickItem.createNew((int)x + bounds.x + 16,(int)y + bounds.y,1));
    		handler.getWorld().getItemManager().addItem(Item.stickItem.createNew((int)x + bounds.x + 32,(int)y + bounds.y+16,1));
    		break;
    	}
